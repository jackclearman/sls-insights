with tab1:
    ...
    if firm_view == "Top Destination Firms":
        ...
        if not display_df.empty:
            ...
            st.info("No detailed data available with current filters.")with tab1:
    # Add dropdown to select between destination and departure firms
    firm_view = st.selectbox("Select View", ["Top Destination Firms", "Top Departure Firms"], index=0)
    ...
    if firm_view == "Top Destination Firms":
        ...
        if len(top_firms) > 0:
            ...
            df_plot = pd.DataFrame({'Firm': top_firms.index, 'Count': top_firms.values})
            ...
            fig = px.bar(
                ...
                x='Firm',
                y='Count',
                labels={"Count": "Number of Attorneys", "Firm": ""}
            )
